...
*Last updated: 2026-09-09.*  **On `main` since v0.8.1, unreleased:** **DD-270** the
Poynting vector as a derived view of any recorded field (`poynting()`, `"S"` in the
component vocabulary), **DD-271** three more readings of an S-matrix (`plot_balance` /
`plot_smith` / `plot_polar`), **DD-272** `result.extrapolate()`, which continues a
truncated record by its own poles, and **DD-273** the surface current `J_s = n × H`
on a conductor, read out of the wall-loss booking.
