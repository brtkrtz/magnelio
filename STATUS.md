...
*Last updated: 2026-09-07.*  **Released v0.7.0** (2026-09-06; a minor
...
* **`fix/paraview-launcher-command` is merged nowhere yet (2026-09-07)** — three commits (`2fada21`, `536ba16`, `974b099`) on top of `5c7cc7c`, green (3090 unit tests, nine ParaView integration tests) and auto-pushed to `private`, but the developer has not said whether to merge.  They make the DD-265 interpreter setting take a *command* (a Flatpak ParaView has no `pvpython` file), say why a bake did not happen instead of returning `state: None` in silence, and give `paraview_open.py` a real message when a plain Python runs it.  Resume: `git switch main`, `git merge --no-ff fix/paraview-launcher-command -m "Merge fix/paraview-launcher-command: … (DD-265)"`, `git push private main`.  The working checkout is on that branch, not on `main`.
* **3D viewer, browser review (DD-261, DD-263, DD-264)** — the toolbar, the volume representations, the mirrored frames and the phase play are gated through trame's state; the toolbar was driven in Chrome for DD-264 (tab name, projection both ways, ruler, PNG with the browser camera, HTML export), the mouse bindings in the help dialog are still read from the vtk.js bundle, not clicked through.
