...
* **3D viewer, browser review (DD-261, DD-263)** — the second toolbar row, the volume representations, the own toolbar (pop-out, help, projection toggle), the mirrored frames and the phase play are gated through trame's state; the DD-261 parts were seen once in Chrome, the DD-263 parts await the developer's look in the browser (the mouse bindings in the help dialog were read from the vtk.js bundle, not clicked through).
