"""AnalysisScatteringTD — S-parameters of a multi-port network, one channel per run.

A scattering analysis derives from :class:`~magnelio.AnalysisTD`: it
takes the same mesh, ports, elements, monitors and wall model, and adds
the analysis band (``f_max``, optionally ``f_min`` / ``n_freq``), an
optional waveform override and the port-pipeline choice.  Its
``run(excited=[...])`` drives one ``(port, mode)`` *channel* per
independent time-domain march — the shared transient engine of the
base class — and merges the columns into a :class:`ScatteringTDResult`
carrying the S-matrix plus the per-excitation V/I time series and the
sampled excitation waveform.
...
back.  Simultaneous excitations of several ports and sources in one
march are :class:`~magnelio.AnalysisTD`'s business; the component
path — building specs, operators, recorder, solver by hand and calling
``compute_s_parameters`` — remains available for custom workflows
neither class covers.  Lumped discrete ports *are* covered here
(``PortSpecLumped``, see "Supported port specs" below).
...
# Design: DD-224 (derives from AnalysisTD, channel runs on the shared
# transient engine), DD-063/DD-064 (port pipeline dispatch, modal
# default for QTEM lines), DD-057 (band-subspace DTBC pipeline), DD-103
# (boundary closure carried on the mesh).
...
from magnelio.analysis.time_domain import (
    AnalysisTD,
...
class ScatteringTDResult(ScatteringResultMixin):
    ...
    def plot_energy(self, *, x: str = "time", floor_db: float | None = None, ax=None):
        ...
        is ``"time"`` (nanoseconds) or ``"step"``; the axis runs from
        ten dB below the criterion (``floor_db`` pins it) to +5 dB;
        ``ax`` draws into existing axes.  Returns ``(fig, ax)``.
        """
        ...
        return plot_energy_traces(traces, energy_stop_db=stop, floor_db=floor_db, x=x, ax=ax)
...
@dataclass
class AnalysisScatteringTD(AnalysisTD):
    ...
    def _check_excitable(self) -> None:
        # A scattering analysis needs ports — a source alone has no
        # S-matrix column; the base class would accept sources.
        if self.ports:
            return
        if not self._ports_from_mesh:
    ...
    def _band_f_min(self) -> float:
        return float(self.f_min)

    def _sibc_band(self) -> tuple[float, float]:
        f_axis = self.f_axis
        return float(f_axis[0]), float(self.f_max)
    ...
    def run(
        ...
        if excitations is not None:
            raise TypeError(
                ...
                "sources are AnalysisTD's business."
            )
        ...
        first = runs[0]
        if len(runs) == 1:
            s_params = first.s_params
            representative = first
        else:
            s_params = SParameterResult.merge([r.s_params for r in runs])
    ...
    def _run_settings(self, **kwargs) -> RunSettings:
        ...
        return super()._run_settings(f_min=self.f_min, n_freq=self.n_freq, **kwargs)
    ...
    def _prepare_excitation_run(
        ...
        """One channel as a prepared run of the shared engine.

        The excited ``(port, mode)`` becomes a single
        :class:`~magnelio.Excitation` at unit amplitude and zero delay
        with the explicit ``waveform`` (or the per-mode default); the
        base class builds the operators, binds the drive and sizes the
        run exactly as it does for a general time-domain run.
        """
        del f_calc  # the base class calculates the modes at self.f_max
        excitation = Excitation(excited_chan[0], mode=excited_chan[1], waveform=waveform)
        return self._prepare_run([excitation], m_eps, m_mu, dt)
    ...
    def _run_one_excitation(
        ...
        prepared = self._prepare_excitation_run(excited_chan, waveform, m_eps, m_mu, dt, f_calc)
        ...
        signals = prepared.recorder.finalize(n_steps_actual=n_actual)
    ...
    def _store_setup(self, dt: float) -> dict:
        setup = super()._store_setup(dt)
        setup["f_min"] = float(self.f_min)
        setup["n_freq"] = int(self.n_freq)
        return setup
    ...
    def _run_streamed(
        ...
        live :class:`~magnelio.io.project._RunSink` attached to the
        solver: V/I, the reference waveform and the energy trace are
        flushed to disk (HDF5-SWMR) as the run proceeds, so a separate
        post-processing script can follow it live.  Each run also
        writes a periodic resume ``checkpoint.h5`` and, on Ctrl-C,
        ...
        for excited_chan in excited_list:
            prepared = self._prepare_excitation_run(
                excited_chan, self.waveform, m_eps, m_mu, dt, self.f_max
            )
            ckpt_interval = (
                checkpoint_interval
            ...
            sink = store.open_run(
                ...
                excitations=[excitation_to_dict(e) for e in prepared.excitations],
                excited=excited_chan,
                dt=dt,
                f_axis=f_axis,
                channels=prepared.recorder.channels,
                port_modes=prepared.port_modes,
                port_normal_dx=prepared.port_normal_dx,
                port_line_params=prepared.port_line_params,
            ...
            self._drive_streamed_solver(solver, sink, _scattering_run_name(excited_chan), path)
    ...
    def _mur_fallback_alternative(self, dt: float) -> str:
        ...
        price = (
            '  Reflection-free alternative: port_model="band" -- the '
            "band-subspace DTBC, measured in the -130 dB class on shielded "
            "microstrip, at about 13x the runtime for a -84 dB floor and "
            "35x at the default subspace rank, without time-domain a()/b() "
            "(methods guide, Ports)."
        )
...
def _resume_scattering(
    ...
    *,
    ...
    "AnalysisScatteringTD"``: rebuilds the analysis from the recipe,
    turns the run's excited channel back into its excitation and hands
    the march to the shared transient resume; the frequency monitors
    are then renormalised with the whole run's reference waveform, as
    at launch.  Parameters mirror the run-time knobs of
...
__all__ = ["AnalysisScatteringTD", "ScatteringTDResult"]
