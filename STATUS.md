...
GitHub Discussions is the feedback channel.  Unit suite 2541 passed /
...
* **Mesh-build speed.**  DD-101/102 (edge prefilter, planar section
  ...
  Lange couplers (3.7 M cells) 32 s, 240 posts (385 k) 22 s, 4 × 4
  patch array with feed (222 k cells) 3 s, 8 × 8 (664 k) 12 s, 16 × 16
  (1.8 M, off-ladder) 92 s; `other` ≤ 1.3 s on every
  row; the pool fires on no ladder row.  Open, in value order: (1) the
  edge pass's ~30 µs per edge is Python per line and edge — a batch
  formulation (every planar row against all carrier lines through its
  box in one compiled call) is the next tier (Lange 16: 6.0 of 32 s,
  8 × 8 array: 4.1 of 12 s); (2) the kernel Booleans of the CSG
  evaluation and the overlap check (Lange 16: `pec_fuse` 4.7 s,
  `overlaps` 3.1 s); (3) the post row's kernel sections at the kernel's
  fixed cost per `BRepAlgoAPI_Section` (6 284 at 1–2 ms, 44 at ~120 ms).
