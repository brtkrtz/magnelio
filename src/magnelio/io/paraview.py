...
def _material_table(materials) -> tuple[list[dict], list[int]]:
    ...
    for mat in materials:
        rgba = tuple(round(float(c), 6) for c in material_color(mat))
...
def export_vtm(
    ...
    table, indices = _material_table([s.material for s in shapes])
...
def _geometry_config(project_path: Path, rel: str) -> tuple[str | None, list[dict]]:
    ...
    *rel* is the path from the session script's own directory, which
    ...
    mats = [_material_from_dict(d) for d, kind in zip(gj["materials"], kinds) if kind != "wire"]
    materials, _ = _material_table(mats)
