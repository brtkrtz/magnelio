...
(DD-231) and **band ports on a DC-reaching axis** (DD-232).  Unit 2790 passed / 4 skipped, integration 447 / 5 skipped (2026-08-30, with
`CUPY_ACCELERATORS=""` — without it the four single-precision GPU
tests fail on nvrtc in the sandbox).  Channels: GitHub, PyPI,
...
`known-bugs.md`.  Measured floors quoted below are regenerated by the
...
* **DD-232** (2026-08-30) — the band excitation is anchored at DC, and the pulse stops scaling with the axis start.  DD-231 closed the port-model default on the fact that the band pipeline cannot even size a default axis, and named the way back: "a band pipeline whose pulse is not tied to the axis start".  The standing explanation for that tie was the Galerkin subspace — built from mode traces on a grid with `f_lo > 0`, so the excitation was thought confined to it by the boundary.  Wrong in three places.  The kernel contour already runs through DC (`sig_hat = -3.7e-9` at the production `n_kernel`), and its dichotomy holds to `sig_hat = -9.1e-13`: the branch point at ζ = 1 is real but the square-root approach keeps the branches 1.3e-5 off the circle.  The static Laplace trace is already *inside* the subspace to 1.24e-8, because the family converges to it (overlap 0.999972 at the band edge, 1.000000 at 20 MHz) — so the planned fix, adding it as a column, was a null operation.  And a boundary built on 2.25–18 GHz absorbs at −83.4 dB at 74.63 MHz, the very axis start DD-231 could not size.  What is band-limited is bookkeeping: `_synthesize_source` zeroes every bin outside the grid where the channel's direction `U(f)` is tabulated, and `_band_setup` sizes the pulse from the narrower roll-off, which near DC is the lower one.  But `U(f→0)` is `track_t`, computed by the DC bootstrap and used only as a tracking anchor.  So `continue_family_to_dc` extends the *fundamental channel's direction table* below the band — interior points measured by the same pencil solve, endpoint in closed form — leaving subspace, projection, channels and both kernels untouched.  The lower roll-off then drops out of the spectrum and the sizing: default axis 1 211 808 → 18 178 steps, inside the budget.  Certified end to end on the coarse two-port fixture: |S11| −117.4 dB worst above 0.5 GHz, −91.2 dB at 33.83 MHz, |S21| within 0.07 dB of modal.  Guarded by overlap ≥ 0.9 so a waveguide fundamental keeps the old path; `band_options={'dc_anchor': False}` restores it everywhere.  It moves feasibility, not cost — on an axis that already sized it is worth 1.3×, and `a()`/`b()` and resume are untouched, so `port_model` stays `"modal"`.
* **DD-231** (2026-08-30) — the port model stays `"modal"`, and the run says what that buys.  DD-229 and DD-230 both ended on the same open question, and DD-230 closed one of its three blockers, so it was due.  The answer is not the cost trade DD-064 made in July: on the canonical case — tutorial 09's shielded microstrip, library defaults — `port_model="auto"` resolves to the band pipeline in 0.16 s and then *refuses to run*, because the default axis starts at 74.6 MHz where the band-limited pulse would need 1 212 141 steps against a budget of 131 072; it wants an axis start of 690 MHz, a factor 9.2 up.  Sized where it does fit, the switch measured 1.9 s → 2688.9 s (1453×) for a floor of −32.9 dB → −137.2 dB, and that comparison already favours the band side by dropping 80 % of the axis.  The other two differences cannot be decided in advance at all: band results have no `a()`/`b()` and no resume, and whether the user wants either is not knowable when the pipeline is chosen — a default must not decide something whose cost depends on what will be asked of the result afterwards.  So the default holds, and the verbose notice stops being a label: it names each fallback channel with the measurement behind it, separates a genuinely inhomogeneous cross-section from one that missed the gate by jitter, states the floor traded away *and* what the run keeps for it, then prices the alternative **for the model at hand**.  That last part fixes a real defect — the old notice recommended `port_model='band'`, which on that same microstrip raises; `_band_min_axis_start` now inverts the same sizing chain `_band_setup` refuses by, so notice and gate cannot disagree (pinned on both sides of the quoted number).  The notice also stopped firing from `AnalysisTD` while naming a keyword only `AnalysisScatteringTD` has.  No numerics moved.
